...
@section('title', 'Edit ' . $siswa->nama_lengkap . ' — PAUD KB Pelangi')
@section('page-title', 'Edit Siswa')

@push('styles')
<style>
/* ══════════════════════════════════════════════════════════
   EDIT SISWA — impeccable pass
   Register: product. Semua token dari app.blade.php (OKLCH).
   Glass: dihapus dari semua card. Hanya pada foto-upload
   overlay (purposeful — float di atas foto).
   Layout: section rhythm, bukan card-per-section.
   Absolute bans: no side-stripe, no gradient-text,
   no glass-as-default, no identical card grids.
══════════════════════════════════════════════════════════ */
...
/* Sections dipisah hanya oleh garis dan padding
   berbeda, bukan card wrapper. Hierarchy via spacing. */
.form-sec {
    padding: 22px 24px;
...
.mode-badge {
    ...
    padding: 2px 9px;
    background: var(--warning-bg);
    border: 1px solid color-mix(in oklch, var(--warning), transparent 55%);
...
/* Glass overlay — justified: floats over foto */
.foto-hover {
...
/* ─── Tab nav (orang tua) ────────────────────── */
/* Tab dipertahankan: UX baik untuk multi-context form.
   Diperhalus: indicator, spacing, transition. */
.tab-strip {
    display: flex; gap: 0;
    border-bottom: 1px solid var(--border);
    margin: 0 -24px 20px;
...
@media (max-width: 600px) {
    .tab-strip { margin: 0 -16px 18px; padding: 0 16px; }
}
...
/* Tab panel dot indicator (ada data) */
.tab-dot {
    display: inline-block;
...
/* ─── Contact input wrapper (dengan prefix icon) ─ */
...
@media (max-width: 600px) { .action-bar { padding: 14px 16px; } }
...
<div style="max-width:720px;margin:0 auto">
    ...
    <div class="bc au d0" style="margin-bottom:16px">
        ...
        <a href="{{ route('siswa.show', $siswa) }}">{{ $siswa->nama_lengkap }}</a>
        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
        </svg>
        <span class="bc-now">Edit</span>
    </div>
        ...
        <div class="form-shell au d1">
            ...
            <div class="form-sec">
                    ...
                    <div class="foto-zone">
                        <label class="foto-btn" for="fotoInput">
                            <template x-if="fotoPreview">
                                <img :src="fotoPreview" class="foto-img" alt="Preview">
                            </template>
                            <template x-if="!fotoPreview">
                                @if($siswa->foto)
                                    <img src="{{ Storage::url($siswa->foto) }}"
                                         class="foto-img" alt="{{ $siswa->nama_lengkap }}">
                                @else
                        ...
                        <div class="foto-meta">
                            ...
                            <label for="fotoInput" class="foto-link">Pilih file baru</label>
                            @error('foto')
                ...
                <div>
                    ...
                    <textarea id="alamat" name="alamat" rows="2"
                              ...
                              class="fc">{{ old('alamat', $siswa->alamat) }}</textarea>
                </div>
            </div>
                ...
                <div class="g3">
                    <div>
                        ...
                        <select id="kelompok" name="kelompok" required
                            ...
                            <option value="KB" {{ old('kelompok', $siswa->kelompok) === 'KB' ? 'selected' : '' }}>KB</option>
                        </select>
                        @error('kelompok')
                        <div class="fld-err">
                            <svg fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
            ...
            <div class="form-sec">
                <div class="sec-label">
                    ...
                    <div>
                        ...
                        <div class="sec-label-sub">Nama, kontak, dan pekerjaan</div>
                    </div>
                </div>
                    ...
                    <div class="g2">
                        ...
                        <div>
                            ...
                            <input id="hubungan_wali" type="text" name="hubungan_wali"
                                   value="{{ old('hubungan_wali', $siswa->hubungan_wali) }}"
            ...
            <div class="action-bar">
                ...
                <div class="action-bar-right">
                    <a href="{{ route('siswa.show', $siswa) }}" class="btn-ghost">Batal</a>
